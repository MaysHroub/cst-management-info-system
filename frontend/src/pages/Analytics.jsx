...
function Analytics() {
    ...
    const simulateBreach = async () => {
        if (confirm("This will artificially age 5 requests to simulate SLA breaches for dashboard verification. Proceed?")) {
    ...
    return (
        <div className="card" style={{ padding: '0', overflow: 'hidden' }}>
            ...
            <div className="p-4 bg-light flex gap-4 text-xs">
                <div className="flex items-center gap-1"><span style={{ width: '10px', height: '10px', background: '#991b1b', borderRadius: '50%' }}></span> High Density Zone</div>
                <div className="flex items-center gap-1"><span style={{ width: '10px', height: '10px', background: '#fbbf24', borderRadius: '50%' }}></span> Low Density Zone</div>
                <div className="flex items-center gap-1"><span style={{ width: '12px', height: '12px', background: '#dc2626', border: '1px solid #fff', borderRadius: '50%' }}></span> Critical Request</div>
            </div>
        </div>
    );
}
                            ...
                            <td>
                                <div className="flex items-center gap-2">
                                    <div style={{ flex: 1, height: '6px', background: 'var(--border)', borderRadius: '3px' }}>
                                        <div style={{ width: `${Math.min(agent.score * 20, 100)}%`, height: '100%', background: 'var(--primary)', borderRadius: '3px' }}></div>
                                    </div>
                                    <span className="text-xs">{agent.score}</span>
                                </div>
                            </td>
                        </tr>
                    ))}
    ...
    return (
        <div className="grid grid-2 gap-4">
            <div className="card">
                ...
                <p className="text-sm text-muted mb-4">Locations with recurrence patterns (2+ incidents)</p>
                <div className="flex flex-col gap-3">
                    {cohorts.map((cohort, i) => (
                        <div key={i} className="p-3 border rounded hover-bg">
                            <div className="flex justify-between items-start">
                                <div>
                                    <strong className="block">{cohort._id || 'Standard Point'}</strong>
                            ...
                            <div className="mt-2 text-xs text-muted flex justify-between">
                                <span>Avg Rating: {cohort.avg_rating?.toFixed(1) || 'N/A'}★</span>
                                <span>Last: {new Date(cohort.last_incident).toLocaleDateString()}</span>
                            </div>
                        </div>
                    ))}
            ...
            <div className="card">
                <div className="alert alert-info mb-4" style={{ background: 'var(--info-bg)', padding: '1rem' }}>
                    <h4 className="text-sm mb-2">Multi-Dimensional Analytics Logic</h4>
                    <p className="text-xs">Data is aggregated using MongoDB <strong>$facet</strong> pipelines to compute backlog, SLA state, and satisfaction distributions in a single pass. <strong>$lookup</strong> is used for real-time agent correlation.</p>
                </div>
                <div className="mt-2">
                    <h4 className="text-sm mb-3">Governance Indicators:</h4>
                    <div className="grid grid-2 gap-2">
                        <div className="p-2 border rounded">
                            <span className="text-xs text-muted block uppercase">BREACH CALC</span>
                            <span className="text-xs">created_at &gt; SLA policy depth</span>
                        </div>
                        <div className="p-2 border rounded">
                            <span className="text-xs text-muted block uppercase">HOTSPOT GROUP</span>
                            <span className="text-xs">geo_hint + category recurrence</span>
                        </div>
                    </div>
                </div>
            </div>
